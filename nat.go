...
type NAT interface {
	...
	// AddMapping maps a port on the local host to an external port.
	AddMapping(protocol string, externalPort, internalPort int, description string, timeout time.Duration) (mappedExternalPort int, err error)
